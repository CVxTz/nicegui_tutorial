...
if [ "$1" = "prod" ]; then
    ...
    uvicorn nicegui_app.main:app --workers 1 --log-level info --port "8080" --host "0.0.0.0"
elif [ "$1" = "dev" ]; then
    echo "Starting Uvicorn server in development mode..."
    # reload implies workers = 1
    uvicorn nicegui_app.main:app --reload --log-level debug --port "8080" --host "0.0.0.0"
else
